...
function testcore {
	...
	#echo $cmdline
	bash -c "timeout -s 9 20s valgrind --log-file=$vglog $cmdline | tr -d ' ' > $out"
	#bash -c "$cmdline > $out "
	RET=$?
	#echo $RET
	...
	then
		...
		$difftool $out $reference > $difflog
		if [ $? != 0 ]
		then 
			FAILED=1
			echo "Case $1 was not properly answered, see $out for your output and $difflog for the differences"
		fi
	fi
...
function testfunc {
	...
	then
		...
		echo -e '...\E[;31m'"\033[1m\"$1\" FAILED\033[0m"
	fi
}
...
testfunc "No Facts" testcore "./$PROG /dev/null cases/nofacts.Q" "cases/nofacts.A.tr" "diff"
...
do
	base="${i%.*}"
	testfunc $base testcore "./$PROG $base.fax $base.q" "$base.a.tr" "diff"
done

# Don't account LEAKs in the three questions above
let "SCORELEAK=TESTLEAK-COMPLEAK"
...
let "SCORE=TESTCASE-TESTFAIL+TESTCASE-COMPCOUNT-SCORELEAK"
