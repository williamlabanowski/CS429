...
while ($line = <INFILE>) {
	...
	if ($name ne "howManyBits") {
		...
		$puzzle_p_points{$name} = "N/A";
		if ($p_points > 0) {
			$puzzle_c_points{$name} = 4;
			$total_c_points += 4;
		}
	}
    }
...
print "\nScore = $tpoints/$trating [$total_c_points/$total_c_rating Correctness + $total_p_points/$total_p_rating Performance] ($tops total operators)\n";
