...
void Print_Cache_Statistics_for_one_cds(CDS *cds)
{
	...
	if(cds->c->victimCache->number_of_cache_entries > 0){
		Print_Cache_Statistics_for_one_cache(cds->c->victimCache);
	}
...
void init_cache(CDS *cds)
{
	...
	//EDITED HERE to malloc for victim cache entries
	if(cds->c->victimCache!=NULL){
		cds->c->victimCache->c_line = CAST(cache_line *, calloc(cds->c->victimCache->number_of_cache_entries, sizeof(cache_line)));
	}
}
...
void delete_one_cache(struct cache *c)
{
    free(c->c_line);
    free(c->name);
    free(c->victimCache->c_line);
    free(c->victimCache->name);
}
