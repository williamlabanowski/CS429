...
{
	swap1(a->valid, b->valid);
    swap1(a->tag, b->tag);
    swap1(a->dirty, b->dirty);

	//swap1(a->replacement_data, b->replacement_data);

	//byte* temp = a->actual_data;
    //a->actual_data = b->actual_data;
	//b->actual_data = temp;	
}
...
{
	//EDITS MADE HERE.  THIS ENTIRE IF STATEMENT
	if(cds->c->victimCache->number_of_cache_entries>0){
		//memory_address victim_cache_address = 
		//	Base_Cache_Address(cds->c->victimCache, cache_address);

		int cache_entry_index = Find_Victim_by_Replacement_Policy(cds->c->victimCache, 0);
		cache_line* cache_entry = &(cds->c->victimCache->c_line[cache_entry_index]);
		
		if(cache_entry->valid && debug){
			fprintf(debug_file, "%s: remove %08X from victim cache\n",cds->name, cache_entry);
		}


		if (cache_entry->dirty)
		{
			evict_dirty_line_from_cache(cds->c->victimCache, cache_entry);
		}

		if (victim_line->dirty)
			cds->c->number_miss_writes += 1;		
		
		swap_cache_lines(cache_entry, victim_line);

		Set_Replacement_Policy_Data(cds->number_of_memory_reference, 
				cds->c->victimCache, cache_entry);
...
void Simulate_Reference_to_Cache_Line(CDS *cds, memory_reference *reference)
{
    ...
    if (cache_entry_index >= 0)
        ...
        {
            //did not find it in the main cache
            ...
            //EDITS MADE HERE
            //search the victim cache for the item
            //instead of doing reference->address here what if we called Base_Cache_Address like he did earlier???  
            //also do we need to only do this when a victim cache exists???
           
            if(cache_entry->valid == TRUE){
				cds->c->victimCache->number_total_cache_access += 1;

                int victim_index = -1;
				if(cds->c->victimCache->number_of_cache_entries>0){
					//memory_address victim_cache_address 
					//	= Base_Cache_Address(cds->c->victimCache, reference->address);
					victim_index = Search_Cache_For(cds->c->victimCache, cache_address);
					//cds->c->number_miss_writes+=1;
				}
                if (victim_index >= 0)
                {
                    /* found it -- record cache hit */
                    //so it wasnt in the main cache but was in the victim.  therefore we swap
                    cache_line* victim_entry = &(cds->c->victimCache->c_line[victim_index]);
                    Found = TRUE;
                    //cds->c->victimCache->number_cache_hits += 1;
                    if (debug) fprintf(debug_file, "%s: found 0x%08X in victim cache %d\n", cds->name, cache_address, victim_index);

					if(cache_entry->dirty == TRUE)
						cds->c->number_miss_writes += 1;
                    swap_cache_lines(victim_entry, cache_entry);
					Set_Replacement_Policy_Data(cds->number_of_memory_reference, cds->c->victimCache, victim_entry);
                }
                else{
                     //did not find it in victim cache or regular cache
                    cds->c->victimCache->number_miss_reads += 1;
                    Found=FALSE;
                }
            }
            else{
            ...
            /* evict victim */
            //we changed the evict_from_cache function to run the same if there is no 
            if (!Found && cache_entry->valid)
                Found = evict_from_cache(cds, cache_entry, cache_address);
            
            
            //what is this doing? do we still need this???  maybe add to the if statement something that says there is no victim cache???
            if (!Found)
                {
