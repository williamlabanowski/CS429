...
void define_key_value_pair(CDS *cds, Token *key, Token *value)
{
    ...
    if ((strcasestr(key->string, "decay") != NULL) && (strcasestr(key->string, "interval") != NULL))
		...
		//EDITED STUFF HERE
		//set the victim cache number of entries according to what we specified
	if(strcasestr(key->string, "victim") != NULL){
		cds->c->victimCache->number_of_cache_entries=atoi(value->string);
		return;
	}
...
CDS *Read_CDS_file_entry(FILE *CDS_file)
{
	...
	//EDITED STUFF HERE
	cds->c->victimCache = CAST(struct cache *,calloc(1,sizeof(struct cache)));
	...
	//EDITED STUFF HERE
	/* victim default values */
	cds->c->victimCache->number_of_cache_entries = 0;
	cds->c->victimCache->number_of_ways = 1;
	cds->c->victimCache->replacement_policy = CRP_FIFO;
	cds->c->victimCache->c_line = NULL;
	cds->c->victimCache->victimCache=NULL;
	...
	//EDITED HERE
	//set the cache line size of the victim to be the same the regular cache
	cds->c->victimCache->cache_line_size = cds->c->cache_line_size;
	
	char main_name[25], victim_name[25];
	if(cds->c->victimCache->number_of_cache_entries > 0){
		snprintf(main_name, sizeof(main_name), "%s main cache", cds->name);
		snprintf(victim_name, sizeof(main_name), "%s victim cache", cds->name);
	}
	else{
		snprintf(main_name, sizeof(main_name), "%s", cds->name);
		snprintf(victim_name, sizeof(main_name), "%s", cds->name);
	}
    
	cds->c->name = remember_string(main_name);
	cds->c->victimCache->name= remember_string(victim_name);
	
	cds->c->victimCache->write_back = cds->c->write_back;
	cds->c->victimCache->number_of_ways= cds->c->victimCache->number_of_cache_entries;
