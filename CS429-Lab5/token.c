...
enum Token_type peek_token_type(Boolean checkFirst)
{
	//printf("\n printing some things: token_index:%d---input_line_len: %d----input_buffer:%s\n", token_index, input_line_length, input_buffer);
	if(checkFirst){
...
void get_token(Token *t)
{
    ...
    enum Token_type a = peek_token_type(FALSE);
    if (a != Tsymbol)
        {
            t->type = a;
